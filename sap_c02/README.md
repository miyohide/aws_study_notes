...
| 種類 | 特徴 | 用途 |
|------|------|------|
| **Cluster（クラスタ）** | **物理的に近いホストにEC2を配置**。超低レイテンシー＆高帯域幅（100Gbps以上）通信が可能 | **HPC（ハイパフォーマンスコンピューティング）、ビッグデータ処理** |
| **Partition（パーティション）** | EC2を複数の「パーティション」に分け、異なるハードウェアに配置（障害の影響を抑える） | **HDFS、Cassandra、HBaseなどの分散システム** |
| **Spread（スプレッド）** | **異なる物理ホスト**にEC2を分散配置し、耐障害性を向上 | **小規模な重要ワークロード（データベース、Webサーバー）** |
...
単一のAmazon Auroraデータベースを複数のAWSリージョンに跨って運用できる。パフォーマンスに影響を与えずにデータをレプリケートし、各リージョンでレイテンシーを提言してローカル読み取りを高速化し、リージョン規模の停止からの災害復旧を実現する。
...
また、一意のElasticなAnycast IPアドレスを二つ割り当てることで、クライアント側には一切の変更なしでアクセス可能。
...
仮想アプライアンス（ファイアウォール、IPS/IDS、DLP など）をスケーラブルかつ可用性の高い形でデプロイするためのロードバランサー。

## **主な特徴**

- **レイヤー3（L3）でのトラフィック転送**
   - GWLBは **Geneve プロトコル** を使用し、トラフィックを仮想アプライアンスへ転送
   - NAT、VPN、セキュリティアプライアンスとの統合が可能

- **仮想アプライアンスの負荷分散**
   - セキュリティアプライアンス（ファイアウォールなど）を複数台用意し、負荷分散して可用性を向上
   - **スケールアウト対応**（需要に応じた仮想アプライアンスの追加が可能）

- **透過的なインライン処理**
   - ユーザーアプリケーションの構成を変更せずに、トラフィックをアプライアンスに転送
   - **Bump-in-the-Wire**（インライン配置）として動作

- **VPC間 & オンプレミス環境と連携**
   - **AWS Transit Gateway (TGW) と統合可能**（複数VPCのセキュリティ管理に最適）
   - **オンプレミスとAWS間のトラフィックもGWLB経由でフィルタリング可能**

# **AWS Step Functions: Standard vs Expressの違い**

AWS Step Functionsには、**Standard** と **Express** の 2種類のワークフローがあります。これらは、異なるユースケースに最適化されており、主にパフォーマンスとコストの面で異なります。以下に、**Standard** と **Express** の違いを説明します。

## **Standard ワークフロー**
### 特徴:
- **長時間実行可能**: 最大 **1年間** 実行できます。長期間のワークフローや、複雑な状態管理が必要な場合に最適です。
- **耐障害性の高い処理**: **リトライ機能、エラーハンドリング、タイムアウト** が強力で、ワークフローの途中でエラーが発生しても、リカバリが可能です。
- **低スループット向け**: 高頻度でのトランザクション処理に最適ではなく、比較的少ないリクエスト数や長時間のプロセスに向いています。

### ユースケース:
- 複雑なビジネスプロセスのオーケストレーション
- 長時間かかるデータ処理やバッチ処理
- 詳細なエラーハンドリングとリトライ機能が必要なシナリオ

## **Express ワークフロー**
### 特徴:
- **短時間実行**: 最大 **5分間** の実行時間（5分以内）。非常に高速な処理を必要とするユースケースに最適です。
- **高スループット**: 高いトランザクション速度をサポートし、低遅延で大量のリクエストを処理できます。
- **低コスト**: 比較的コストが低く、スループットが高いトランザクション向けで有利です。
- **簡素なエラーハンドリング**: Standardワークフローに比べ、エラーハンドリングやリトライ処理が少しシンプルです。

### ユースケース:
- 高速なデータ処理や通知処理
- 短時間のバッチ処理
- リアルタイム処理や短期間で完了するタスクのオーケストレーション

## **Standard vs Express の比較**

| 特徴                         | **Standard ワークフロー**                         | **Express ワークフロー**                          |
|----------------------------|--------------------------------------------------|---------------------------------------------------|
| **実行時間の制限**              | 最大 1年                                          | 最大 5分                                          |
| **処理スループット**            | 低スループット（少数のトランザクション）                | 高スループット（大量のトランザクション）                |
| **エラーハンドリング/リトライ**  | 高度なエラーハンドリング、リトライ機能、タイムアウト   | 基本的なエラーハンドリング、リトライは少ない         |
| **コスト**                    | 高コスト（特に長時間実行される場合）                   | 低コスト（高頻度の短期間処理）                      |
| **ユースケース**               | 複雑なプロセスオーケストレーション、長時間処理、ビジネスロジック | 高スループットが必要な短期間のタスク処理            |
