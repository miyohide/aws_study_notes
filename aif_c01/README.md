...
| **k-NN（k-近傍法, k-Nearest Neighbors）** | 新しいデータに対し、学習データの中から最も近い k 個のデータを探し、その多数決や平均で予測。 | - 学習フェーズがほぼ不要（記憶型）<br>- 単純で直感的<br>- データスケーリングが重要<br>- 予測時の計算コストが高い | - 類似画像検索<br>- レコメンドシステム<br>- 類似症例に基づく診断       |
...
温度値を下げると、より一貫性のある応答が生成される。温度値を上げると、ランダム性が増し一貫性が低下する。
