...
| モデル名                                 | 概要                                              | 特徴                                                                 | 具体例                                           |
| ------------------------------------ | ----------------------------------------------- | ------------------------------------------------------------------ | --------------------------------------------- |
| **ディシジョンツリー（Decision Tree）**         | 特徴量に基づいて条件分岐を繰り返し、木構造で分類や回帰を行うモデル。              | - 可視化・解釈が容易<br>- 前処理が少なくても動作<br>- 非線形関係を扱える<br>- 過学習しやすい（深い木）      | - 顧客の購買行動の分類<br>- 融資審査（貸付可否）<br>- 医療診断（症状→病名） |
| **線形回帰（Linear Regression）**          | 入力変数と出力変数の線形関係を仮定し、最適な直線（平面）を求めて数値を予測。連続値の予想に使われる。          | - 実装が容易・解釈が明確<br>- 外れ値に弱い<br>- 線形関係しか扱えない                          | - 家賃予測<br>- 売上予測<br>- 給与の推定（経験年数など）           |
| **ロジスティック回帰（Logistic Regression）**   | 線形結合した特徴量をシグモイド関数で確率に変換し、2値または多クラス分類を行う。        | - 出力が確率として解釈可能<br>- シンプルな分類モデル<br>- 線形的な境界しか扱えない                   | - スパム判定<br>- 顧客離脱予測<br>- 疾病の有無分類              |
| **ニューラルネットワーク（Neural Network）**      | 複数層のノード（ニューロン）で非線形な関係を学習し、複雑なパターンを表現するモデル。      | - 非線形・高次元データに強い<br>- 大量データが必要<br>- 学習コストが高い<br>- 解釈が難しい（ブラックボックス）  | - 画像認識<br>- 音声認識<br>- 自然言語処理（翻訳・生成など）         |
| **k-NN（k-近傍法, k-Nearest Neighbors）** | 新しいデータに対し、学習データの中から最も近い k 個のデータを探し、その多数決や平均で予測。<br>「似ているデータは似た結果を持つ」という直感的な仮定に基づく。つまり、未知のデータ点に対して、すでにラベル（答え）がわかっているデータの中から、距離が近いk個のデータ（近傍）を探し、その近傍の情報から予測を行う手法。 | - 学習フェーズがほぼ不要（記憶型）<br>- 単純で直感的<br>- データスケーリングが重要<br>- 予測時の計算コストが高い | - 類似画像検索<br>- レコメンドシステム<br>- 類似症例に基づく診断       |
...
## ROUGE・BLEU・BERTScoreの違い

| 指標                                                               | 主な評価対象              | 評価の観点                                        | 手法の概要                                              | 特徴・強み                              | 弱点                                    |
| ---------------------------------------------------------------- | ------------------- | -------------------------------------------- | -------------------------------------------------- | ---------------------------------- | ------------------------------------- |
| **ROUGE**<br>(Recall-Oriented Understudy for Gisting Evaluation) | 主に**要約タスク**         | **再現率（Recall）重視**<br>＝参照文の内容をどれだけ「拾えた」か      | n-gramやLCS（最長共通部分列）などの**文字列の重なり**を測定               | 要約性能の比較に有効<br>参照文をどれだけ再現できたかを評価できる | 同義語や文構造の違いを認識できない                     |
| **BLEU**<br>(Bilingual Evaluation Understudy)                    | 主に**機械翻訳タスク**       | **精度（Precision）重視**<br>＝生成文の内容がどれだけ参照文に含まれるか | n-gram一致を測定し、文の長さにペナルティ（Brevity Penalty）を加味        | 翻訳評価の事実上の標準<br>単語順序や長さも考慮          | 言い換え（paraphrase）を正しく扱えない<br>意味的な評価が弱い |
| **BERTScore**                                                    | 要約・翻訳・自由生成など**幅広く** | **意味的一致（semantic similarity）**               | BERTなどの**文脈埋め込み（embedding）**を用い、単語ベクトル間のコサイン類似度を算出 | 同義語や語順の違いを理解できる<br>意味的に近い文も高評価     | モデル依存・計算コストが高い<br>再現性（モデルバージョン差）が課題   |
